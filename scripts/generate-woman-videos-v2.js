...
const POLL_INTERVAL_MS = 30000; // 30 seconds per poll (faster for testing)
const MAX_POLLS = 900; // 900 * 30s = 7.5 hours = extended timeout
...
async function processImage() {
  ...
  form.append('image', imageBuffer, { filename: 'Woman-onbaoridng.jpg' });
  ...
  try {
    ...
    if (!res.ok) {
      console.error(`[processImage] Response:`, JSON.stringify(data, null, 2));
...
async function generateVideo(imageUrl, preset) {
  ...
  try {
    ...
    console.log(`[generateVideo] Sending: ${JSON.stringify(body)}`);
    ...
    if (!res.ok) {
      console.error(`[generateVideo] Response:`, JSON.stringify(data, null, 2));
