...
const POLL_INTERVAL_MS = 240000; // 240 seconds = 4 minutes per poll
const MAX_POLLS = 900; // 900 * 4 min = 60 hours = extended timeout
...
// Process image once (shared for both videos)
async function processImage() {
  ...
  const imageData = fs.readFileSync(WOMAN_IMAGE);
  const FormData = require('form-data');
  const form = new FormData();
  form.append('user_id', USER_ID);
  form.append('image', imageData, 'Woman-onbaoridng.jpg');
...
async function generateVideo(imageUrl, preset) {
  ...
  try {
    const res = await fetchJSON(`${RENDER_API}/generate-video`, {
      method: 'POST',
      body: {
        user_id: USER_ID,
        image_url: imageUrl,
    ...
    if (res.status !== 200) throw new Error(`generate-video failed: ${res.status}`);
...
// Poll for completion (extended: up to 900 attempts, 240s per poll = 60 hours max)
async function pollUntilDone(taskId, maxAttempts = MAX_POLLS) {
  ...
  for (let i = 0; i < maxAttempts; i++) {
    try {
      const res = await fetchJSON(`${RENDER_API}/video-status/${taskId}`);
      const status = res.data.status || res.data.state;
      
      if (status === 'completed' || status === 'succeed') {
        const videoUrl = res.data.video_url || res.data.output_video_url;
  ...
  throw new Error(`Polling timeout after ${maxAttempts} attempts (${(maxAttempts * POLL_INTERVAL_MS / 1000 / 3600).toFixed(1)} hours)`);
}
...
async function updateSupabase(videoName, r2Url) {
  ...
  try {
    const res = await fetchJSON(`${SUPABASE_URL}/rest/v1/demo_videos?name=eq.${videoName}`, {
      method: 'PATCH',
      headers: {
...
// Main
(async () => {
  console.log('\n🎬 GROOVE AI WOMAN VIDEO GENERATION (EXTENDED TIMEOUT)\n');
  console.log(`User ID: ${USER_ID}`);
  console.log(`Image: ${WOMAN_IMAGE}`);
  console.log(`Polling: ${MAX_POLLS} attempts × ${POLL_INTERVAL_MS / 1000 / 60} min = ${(MAX_POLLS * POLL_INTERVAL_MS / 1000 / 3600).toFixed(1)} hours max\n`);
  ...
  try {
    ...
    if (allDone) {
      ...
      console.log('❌ SOME VIDEOS FAILED\n');
      process.exit(1);
    }
